  ...
  useEffect(() => {
    ...
    const loadStockByNursery = async () => {
      try {
        ...
        // Plant (Option B) - depends on isUniqueInstance
        const isUnique = Boolean(selectedItem.isUniqueInstance);
        console.log('[StockByNursery] plant branch', { requestId, cacheKey, isUnique });

        if (!isUnique) {
          // Common plant -> follow /products/[plantId] flow: GET /shop/plants/{id}/common-nurseries (NurseryResponse[])
        ...
        // Unique instance plant -> follow /products/[plantId] flow: GET /plants/{id}/nurseries (NurseryResponse[])
