...
import { getProgressByRegistrationId, markServiceProgressReviewed } from '@/lib/api/careServiceService';
...
interface ManagerServiceProgressSectionProps {
...
interface ProgressItemProps {
  ...
  index: number;
  total: number;
  onViewFeedback: (item: NurseryServiceScheduleItem) => void;
  onMarkReviewed: (item: NurseryServiceScheduleItem) => void;
  markingReviewedId: number | null;
}

function ProgressItem({ item, index, total, onViewFeedback, onMarkReviewed, markingReviewedId }: ProgressItemProps) {
  ...
  const isMarking = markingReviewedId === item.id;
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
            ...
            {item.customerFeedback && (
              item.isReviewed ? (
                <Chip
                  icon={<DoneAllIcon sx={{ fontSize: 13 }} />}
                  label="Reviewed"
                  color="success"
                  size="small"
                ...
                <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                  ...
                  {!item.isReviewed && (
                    <Button
                      size="small"
                      variant="contained"
                      startIcon={<MarkChatReadIcon sx={{ fontSize: 14 }} />}
...
export default function ManagerServiceProgressSection({ registrationId, enabled, onAnyReviewed }: ManagerServiceProgressSectionProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setLoading(true);
    getProgressByRegistrationId(registrationId, false)
      .then((data) => {
        if (!cancelled) {
  ...
  const handleMarkReviewed = async (item: NurseryServiceScheduleItem) => {
    setMarkingReviewedId(item.id);
    try {
      const updated = await markServiceProgressReviewed(item.id);
      setProgresses((prev) =>
        prev.map((p) => (p.id === updated.id ? { ...p, isReviewed: true } : p))
      );
  ...
  const unreviewedCount = progresses.filter((p) => p.customerFeedback && !p.isReviewed).length;
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          {unreviewedCount > 0 && (
            <Chip
              icon={<ChatBubbleOutlineIcon sx={{ fontSize: 13 }} />}
              label={`${unreviewedCount} unreviewed`}
              color="warning"
              size="small"
      ...
      <FeedbackModal
        ...
        title={targetIndex >= 0 ? `Session ${targetIndex + 1} — Customer Feedback` : 'Customer Feedback'}
